...
def edit_distance(s1, s2):
    m, n = len(s1), len(s2) # длины строк
    if m < n:
        return edit_distance(s2, s1)

    prev = list(range(n + 1))  # Заменяет создание таблицы

    for i, ch1 in enumerate(s1, 1):  # убирает индексацию
        curr = [i] # Заменяет создание таблицы

        for j, ch2 in enumerate(s2, 1):
            # Заметим, что d[i][j] зависит только от предыдущей строчки и столбца
